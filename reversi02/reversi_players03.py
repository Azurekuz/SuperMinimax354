...
class MinimaxComputerPlayer:
    ...
    def minimax(self, board, depth, cur_symbol, last_move):
        emuboard = copy.deepcopy(board)
        emuboard.make_move(cur_symbol, last_move)
        ...
        if cur_symbol == self.symbol:
            max_val = None
            for move in emuboard.calc_valid_moves(cur_symbol):
                min_max_val = self.minimax(emuboard, depth - 1, board.get_opponent_symbol(self.symbol), move)
                if max_val is None:
                    max_val = min_max_val
                elif min_max_val is not None:
                    max_val = self.min_max_eval(max_val, min_max_val, False)
            return max_val
        elif cur_symbol != self.symbol:
            min_val = None
            for move in board.calc_valid_moves(cur_symbol):
                min_max_val = self.minimax(emuboard, depth - 1, self.symbol, move)
                if min_val is None:
                    min_val = min_max_val
                elif min_max_val is not None:
                    min_val = self.min_max_eval(min_val, min_max_val, True)
            return min_val
